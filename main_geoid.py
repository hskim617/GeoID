...
from datasets.initialization import get_dataset, get_test_dataset, get_tta_dataset
import pipeline.exp_geoid as exp

def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('-c', '--config',
                        default="configs/geoid/config_train_kitti_source.yaml",
                        type=str,
                        help="Path to config file")
    parser.add_argument('-ld', '--logdir', default='kitti_source_train')
    parser.add_argument('-sd', '--savedir', default='logs')
    parser.add_argument('-en', '--expname', default='ExpGeoID')
...
def train(cfg, args):
    ...
    if args.debug:
        ...
        logger = TensorBoardLogger(args.savedir, name=cfg.log_dir, default_hp_metric=True)

    source_name = cfg.generalization_params.source
    
    checkpoint_callback = ModelCheckpoint(
        filename=source_name + "-{epoch}-{step}",
        monitor=f'valid/mIoU/{source_name}',
        save_last=True,
        save_top_k=3,
        mode='max',
        )
        
    trainer = Trainer(max_steps=cfg.train_params.max_steps,
                      gpus=1, 
                      logger=[logger],
...
def tta(cfg, args):
    test_dataset = get_tta_dataset(cfg=cfg, debug=args.debug)
