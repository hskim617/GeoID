...
def get_common_color_map(label_mapping, return_torch_tenor=False, return_numpy_ndarray=False):
    ...
    common_color_map = dict()
    for i in sorted(list(commonyaml['color_map'].keys()))[::-1]:
        common_color_map[i] = commonyaml['color_map'][i][::-1] # bgr -> rgb

    if return_torch_tenor:
        common_color_map_ten = torch.zeros(len(common_color_map), 3)
        for i in sorted(list(common_color_map.keys())):
            common_color_map_ten[i] = torch.tensor(common_color_map[i])
        return common_color_map_ten
    
    if return_numpy_ndarray:
        common_color_map_np = np.zeros((len(common_color_map), 3))
        for i in sorted(list(common_color_map.keys())):
            common_color_map_np[i] = np.array(common_color_map[i])
        return common_color_map_np
...
def get_nuScenes_color_map(label_mapping, return_torch_tenor=False, return_numpy_ndarray=False):
    with open(label_mapping, 'r') as stream:
        nuscenesyaml = yaml.safe_load(stream)
    nuScenes_color_map = dict()
    for i in sorted(list(nuscenesyaml['learning_map'].keys()))[::-1]:
        nuScenes_color_map[nuscenesyaml['learning_map'][i]] = nuscenesyaml['color_map'][i][::-1] # bgr -> rgb

    if return_torch_tenor:
        nuScenes_color_map_ten = torch.zeros(len(nuScenes_color_map), 3)
        for i in sorted(list(nuScenes_color_map.keys())):
            nuScenes_color_map_ten[i] = torch.tensor(nuScenes_color_map[i])
        return nuScenes_color_map_ten
    
    if return_numpy_ndarray:
        nuScenes_color_map_np = np.zeros((len(nuScenes_color_map), 3))
        for i in sorted(list(nuScenes_color_map.keys())):
            nuScenes_color_map_np[i] = np.array(nuScenes_color_map[i])
        return nuScenes_color_map_np

    return nuScenes_color_map

COLOR_MAP_KITTI = get_SemKITTI_color_map('configs/label_mapping/semantic-kitti.yaml', return_numpy_ndarray=True)
COLOR_MAP_NUSC = get_nuScenes_color_map('configs/label_mapping/nuscenes.yaml', return_numpy_ndarray=True)
COLOR_MAP = get_common_color_map('configs/label_mapping/common_map_merged.yaml', return_numpy_ndarray=True)
...
def get_semcolor_nusc(pt_label):
    if pt_label.ndim == 2:
        pt_label = pt_label.squeeze(1)
        
    pt_color = np.take(COLOR_MAP_NUSC, pt_label, axis=0).astype(np.uint8)
    return pt_color
